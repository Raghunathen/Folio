...
class FluidVelocityTracker(context: Context) : VelocityTracker {
    ...
    override fun computeCurrentVelocity(units: Int, maxVelocity: Float) {
        // A plain tap can register just one movement sample, which is not enough to derive any
        // velocity from (calculateRecurrenceRelationVelocity requires at least two points and
        // throws otherwise). Nothing moved, so the velocity is simply zero.
        if (times.size < 2) {
            calculatedVelocity = PointF(0f, 0f)
            return
        }
    ...
    private fun calculateRecurrenceRelationVelocity(
        ...
        return if (currentVelocity != null) {
            ...
            // Every sample was discarded above because all the consecutive points shared the
            // same timestamp - which is exactly what a quick tap looks like, since its events can
            // all land within the same millisecond. There is no velocity to compute from that.
