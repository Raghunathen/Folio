...
import com.raghu.folio.logic.utils.CircularShuffleOrder
import com.raghu.folio.logic.utils.LastPlayedManager
import com.raghu.folio.logic.utils.LrcUtils.extractAndParseLyrics
import com.raghu.folio.logic.utils.LrcUtils.loadAndParseLyricsFile
import com.raghu.folio.logic.utils.MediaStoreUtils
import com.raghu.folio.logic.utils.exoplayer.EndedWorkaroundPlayer
...
/**
 * [FolioPlaybackService] is a server service.
 * It's using exoplayer2 as its player backend.
 */
@androidx.annotation.OptIn(UnstableApi::class)
class FolioPlaybackService : MediaLibraryService(), MediaSessionService.Listener,
    ...
    companion object {
        ...
        var instanceForWidgetAndLyricsOnly: FolioPlaybackService? = null
    }
        ...
        if (mediaId != null) {
            ...
            if (elapsedMs > 0) {
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    try {
                        com.raghu.folio.logic.data.db.AppDatabase.getInstance(applicationContext)
    ...
    private fun maybeBackupListeningStats() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val folder = com.raghu.folio.logic.utils.ListeningStatsBackupUtils
                ...
                if (com.raghu.folio.logic.utils.ListeningStatsBackupUtils
                ...
                .setBitmapLoader(object : BitmapLoader {
                    ...
                    override fun loadBitmap(
                        ...
                        return CallbackToFutureAdapter.getFuture { completer ->
                            imageLoader.enqueue(
                                ImageRequest.Builder(this@FolioPlaybackService)
                                    .data(uri)
                                    .allowHardware(false)
                                    .target(
                                        onStart = { _ ->
    ...
    override fun onForegroundServiceStartNotAllowedException() {
        ...
        if (mayThrowForegroundServiceStartNotAllowed()) {
            nm.notify(NOTIFY_ID, NotificationCompat.Builder(this, NOTIFY_CHANNEL_ID).apply {
                ...
                setContentTitle(this@FolioPlaybackService.getString(R.string.fgs_failed_title))
                setContentText(this@FolioPlaybackService.getString(R.string.fgs_failed_text))
                setContentIntent(
                    PendingIntent.getActivity(
                        this@FolioPlaybackService,
                        PENDING_INTENT_NOTIFY_ID,
                        Intent(this@FolioPlaybackService, MainActivity::class.java)
