...
class CircularShuffleOrder private constructor(
    ...
    companion object {
        private const val TAG = "FolioShuffleOrder"
    ...
    class Persistent private constructor(private val seed: Long, private val data: IntArray?) {
        ...
        fun toFactory(controller: MediaController): (Int) -> ((CircularShuffleOrder) -> Unit) -> CircularShuffleOrder {
            return { c ->
                // The persisted shuffle order can go stale relative to the live playlist (eg. a
                // song was queued/removed after the last save() but before this restore/resume
                // applies it) - replaying it verbatim would desync indices, so fall back to a
                // fresh order sized to the actual playlist instead of crashing playback.
                if (data != null && controller.mediaItemCount == data.size) {
                    { CircularShuffleOrder(it, data, seed) }
                } else {
                    if (data != null) {
                        Log.w(
                            TAG, "CircularShuffleOrder.Persistable: " +
                                    "${controller.mediaItemCount} != ${data.size}, discarding stale shuffle order"
                        )
                    }
                    { CircularShuffleOrder(it, c, controller.mediaItemCount, seed) }
                }
            }
        }
    }
