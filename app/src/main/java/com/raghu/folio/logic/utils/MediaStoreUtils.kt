...
import com.raghu.folio.logic.getColumnIndexOrNull
import com.raghu.folio.logic.hasAlbumArtistIdInMediaStore
import com.raghu.folio.logic.hasImagePermission
import com.raghu.folio.logic.hasImprovedMediaStore
import com.raghu.folio.logic.hasScopedStorageV1
import com.raghu.folio.logic.hasScopedStorageV2
import com.raghu.folio.logic.hasScopedStorageWithMediaTypes
import com.raghu.folio.logic.putIfAbsentSupport
import com.raghu.folio.logic.utils.DatabaseUtils.getPrivatePlaylist
import com.raghu.folio.logic.utils.LrcUtils.Label
...
object MediaStoreUtils {
    ...
    // Last-resort year fallback for a file MediaStore's own scanner didn't populate either YEAR
    // or DATE_TAKEN for (both come from MediaStore's own tag parser, which can miss less common
    // tag layouts). MediaMetadataRetriever parses the file's embedded tags directly - ID3v1/v2
    // (MP3), Vorbis comments (FLAC/OGG), MP4 atoms (M4A/AAC) etc. - independent of MediaStore's
    // own extraction, so it catches files MediaStore's columns missed.
    // Deliberately NOT called during the library scan (getAllSongs() below) - opening and
    // parsing every file that way is expensive across a whole library and visibly slows scanning
    // down. Called on demand instead, only for the one song the details page is showing.
    fun extractYearFromTags(path: String): Int? {
        return try {
            MediaMetadataRetriever().use { retriever ->
                retriever.setDataSource(path)
                val raw = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_YEAR)
                    ?: retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DATE)
                Regex("\\d{4}").find(raw ?: return@use null)?.value?.toIntOrNull()
            }
        } catch (e: Exception) {
            null
        }
                ...
                val song = MediaItem
                    ...
                    .setMediaMetadata(
                        MediaMetadata
                            ...
                            // The legacy YEAR column misses tags some taggers write (e.g. ID3v2.4
                            // TDRC), leaving it 0/null even though the file has a valid date.
                            // DATE_TAKEN comes from MediaStore's newer, more complete tag parser,
                            // so fall back to it before giving up on the release year. (A further
                            // fallback that reads the file's tags directly exists too, but it's
                            // deliberately not run here - see extractYearFromTags() - it's only
                            // used on demand by the details page, not during the library scan.)
                            .setReleaseYear(year ?: dateTakenYear)
