    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences?, key: String?) {
        when (key) {
            "theme_mode" -> {
                ...
                // Dynamic color's precondition is evaluated on activity creation, so force a
                // recreate to pick up the change immediately (night mode alone may not trigger one).
