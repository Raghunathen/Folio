...
open class BaseDecorAdapter<T : BaseAdapter<*>>(
        ...
        holder.sortButton.setOnClickListener { view ->
            ...
            layoutMap.forEach {
                popupMenu.menu.findItem(it.key).isVisible = adapter.ownsView && !isPrivateLayout &&
                        (it.value != BaseAdapter.LayoutType.GRID || adapter.supportsGridLayout)
            }
    ...
    /**
     * Hides the whole decor row (song count, play/shuffle, sort). Used while a search is active,
     * where a total and "play everything" buttons describe the unfiltered library rather than the
     * results on screen, so showing them is just misleading.
     */
    var hidden = false
        set(value) {
    ...
    override fun getItemCount(): Int = if (hidden) 0 else 1
