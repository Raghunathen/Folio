...
import com.raghu.folio.logic.getStringStrict
import com.raghu.folio.logic.ui.DefaultItemHeightHelper
import com.raghu.folio.logic.ui.ItemHeightHelper
import com.raghu.folio.logic.ui.MyRecyclerView
import com.raghu.folio.logic.ui.coolCrossfade
import com.raghu.folio.logic.utils.FileOpUtils
import com.raghu.folio.logic.utils.MediaStoreUtils
import com.raghu.folio.ui.MainActivity
import com.raghu.folio.ui.components.CustomGridLayoutManager
import com.raghu.folio.ui.components.GridPaddingDecoration
import com.raghu.folio.ui.fragments.AdapterFragment
...
abstract class BaseAdapter<T>(
    ...
    protected val decorAdapter by lazy { createDecorAdapter() }
    open override val concatAdapter by lazy { ConcatAdapter(decorAdapter, this) }
    open override val itemHeightHelper by lazy {
        DefaultItemHeightHelper.concatItemHeightHelper(decorAdapter, { decorAdapter.itemCount }, this)
    }
    ...
    // Some adapters (e.g. SongAdapter with its inline search header) can crash GridLayoutManager
    // when nested inside the tab ViewPager2 - see the layoutType setter below.
    open val supportsGridLayout: Boolean = true

    var layoutType: LayoutType? = null
        @SuppressLint("NotifyDataSetChanged")
        set(rawValue) {
            val value = if (rawValue == LayoutType.GRID && !supportsGridLayout)
                LayoutType.COMPACT_LIST else rawValue
            ...
            if (value != null && ownsView) {
                // Only an actual GRID layout should ever become a multi-column
                // GridLayoutManager - a plain list (Songs, etc.) must stay single-column
                // regardless of orientation, it was previously also forced into a 2-column grid
                // in landscape here, which is what split the Songs page into two columns on
                // rotation.
    ...
    override fun onBindViewHolder(
        ...
        holder.moreButton.setOnClickListener {
            ...
            // Some onMenu() overrides (eg. SongAdapter's) show their own UI directly and never
            // touch this popup - only show it if it actually got inflated with something.
            if (popupMenu.menu.size() > 0) popupMenu.show()
        }
    ...
    // Extra header adapters prepended before decorAdapter in concatAdapter (default: 0)
    protected open val extraHeaderCount: Int = 0

    override fun getPopupText(view: View, position: Int): CharSequence {
        // position here refers to pos in ConcatAdapter(!)
        // offset = decorAdapter (0 while it's hidden, e.g. during a search) + any extra headers
        // prepended before it
        val offset = decorAdapter.itemCount + extraHeaderCount
        return (if (position >= offset)
            sorter.getFastScrollHintFor(list[position - offset], sortType)
        else null) ?: "-"
    }
