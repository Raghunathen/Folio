...
class FolderAdapter(
    ...
    override val concatAdapter: ConcatAdapter by lazy {
        ConcatAdapter(searchHeaderAdapter, this, folderPopAdapter, folderAdapter, songAdapter)
    }
    ...
    private fun applyFolderSearch() {
        if (folderSearchQuery.isBlank()) {
            folderAdapter.updateListSync(fullFolderList)
            songAdapter.updateList(fullSongList, now = true, true)
            return
        }
        // Same forgiving matching as the Songs tab - see SearchMatcher.
        val tokens = SearchMatcher.tokenize(folderSearchQuery)
        val filteredFolders = fullFolderList.filter {
            SearchMatcher.matches(SearchMatcher.squash(it.folderName), tokens)
        }
        val filteredSongs = fullSongList.filter {
            SearchMatcher.matches(SearchMatcher.haystackOf(it), tokens)
        }
        folderAdapter.updateListSync(filteredFolders)
        songAdapter.updateList(filteredSongs, now = true, true)
    }
    ...
    private fun update(invertedDirection: Boolean?) {
        ...
        val doUpdate = { canDiff: Boolean ->
            ...
            fullFolderList = item?.folderList?.values ?: listOf()
            fullSongList = item?.songList ?: listOf()
            folderSearchQuery = ""
            searchHeaderAdapter.clearText()
            folderAdapter.updateList(fullFolderList, canDiff)
            songAdapter.updateList(fullSongList, now = true, false)
        }
        recyclerView.let {
    ...
    override fun getPopupText(view: View, position: Int): CharSequence {
        var newPos = position - 1  // skip search header at position 0
        if (newPos < 0) return "-"
    ...
    private class FolderListAdapter(
        ...
        fun updateListSync(newCollection: Collection<MediaStoreUtils.FileNode>) {
            val newList = newCollection.toList()
            val diffResult = DiffUtil.calculateDiff(DiffCallback(folderList, newList))
            folderList = newList
