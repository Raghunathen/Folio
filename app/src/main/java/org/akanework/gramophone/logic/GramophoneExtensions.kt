...
fun View.enableEdgeToEdgePaddingListener(
    ...
    if (this is AppBarLayout) {
        ...
        // The expanded (big-title) state of these collapsing toolbars is sized for portrait -
        // in landscape there's much less height to spare, and starting fully expanded ate a huge
        // chunk of the screen before showing any real content. Shrinking the toolbar's own
        // layout_height instead broke the Medium style's internal collapse math (it assumes a
        // specific expanded/collapsed height ratio), so start already scrolled to the collapsed
        // state instead - same mechanism as the user manually scrolling, just triggered upfront.
    ...
    this.setOnMenuItemClickListener {
        when (it.itemId) {
            ...
            R.id.exit -> {
                ...
                activity.stopService(Intent(activity, GramophonePlaybackService::class.java))
